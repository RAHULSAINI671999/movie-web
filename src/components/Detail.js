...
const Detail = () => {
  ...
  return (
    ...
    <div className='p-4 mt-4 w-full flex flex-col md:flex-row items-center md:items-start justify-center'>
     { loading ? <div className='h-96 flex w-full justify-center items-center'><ThreeCircles  height={32} color="white" /></div> :
      <>
  <img  className='h-96 block md:sticky  top-24' src={data.image} alt=''/>
  <div className='md:ml-4 ml-0 w-full md:w-1/2 '>
    <h1 className='text-2xl font-bold text-red-400'>{data.title}<span className='text-xl'>({data.year})</span></h1>
